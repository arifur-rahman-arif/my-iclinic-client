...
const Card = ({ image, title, description, list, borderColor, cardBg, index }: CardInterface): JSX.Element => {
    ...
    if (index === 2) {
        ...
        imageBorder = 'sm:rounded-tr-[7.5rem]';
    }
    if (index === 3) {
        cardBorder = 'sm:rounded-tl-[10rem]';
        bodyBorder = 'sm:rounded-tl-0';
        imageBorder = 'rounded-tr-primary sm:rounded-tl-[7.5rem]';
    }
    ...
    return (
        ...
        >
            <Image
                className={`max-h-[23.5rem] rounded-tl-primary rounded-tr-primary ${imageBorder}`}
                src={image.url}
                width={543}
                height={235}
                alt={''}
