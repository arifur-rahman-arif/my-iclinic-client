    ...
    return (
        <Section id="comparison-table">
            <Container className="grid content-start gap-12">
                ...
                <div className="grid grid-cols-[1fr_2rem] overflow-x-auto rounded-radius2 border border-solid border-[#EAECF0] p-8 pb-16  pr-0 sm:grid-cols-[1fr_3rem] sm:p-12 sm:pr-0 lg:pb-12">
                        ...
                        <thead className="">
                            <tr>
                                {table[0].tableRows.map((item: any, key: any) => (
                                    <th
                                        key={key}
                                        dangerouslySetInnerHTML={{ __html: stripInitialTags(item.value) }}
                                    ></th>
                                ))}
                            </tr>
                        </thead>
                                ...
                                <tr key={key}>
                                    {row.tableRows.map((item: any, index: any) => (
                                        <td key={index}>
                                            {item.value === '1' ? (
                                                <Image
                                                    src="/images/icons/icon-tick-light.svg"
                                                    alt="tick"
                                                    width={24}
                                                    height={24}
                                                />
                                            ) : (
                                                HTMLReactParser(stripInitialTags(item.value))
                                            )}
                                        </td>
                                    ))}
                                </tr>
