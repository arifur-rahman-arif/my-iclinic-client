...
const TabHeadings = (): JSX.Element => {
    ...
    return (
        <div className="flex items-center justify-items-center gap-8 sm:gap-12 md:gap-24">
            {appCtx?.tabs ?
                appCtx.tabs.map((tab, index) => (
                    <button
                        className="grid gap-[0.8rem] font-latoBold text-[1.6rem] leading-8 md:text-[2.4rem] md:leading-[3.2rem]"
                        onClick={() => {
