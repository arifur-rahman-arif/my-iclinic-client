    ...
    return (
        <Page title={heading} description={subheading} seo={seo} yoastJson={yoastJson}>
            ...
            <MastheadICL
                ...
                cardList={data?.masthead?.cardList}
            />
            ...
            props: {
                ...
                data: {
                    ...
                    masthead: {
                        ...data?.acf?.masthead,
                        largeImage: {
                            ...(data?.acf?.masthead?.largeImage && {
                                src: data.acf.masthead?.largeImage.url,
                                width: data.acf.masthead?.largeImage.width,
                                height: data.acf.masthead?.largeImage.height
                            })
                        },
                        smallImage: {
                            ...(data?.acf?.masthead?.smallImage && {
                                src: data.acf.masthead?.smallImage.url,
                                width: data.acf.masthead?.smallImage.width,
                                height: data.acf.masthead?.smallImage.height
                            })
                        },
                        cardList: data?.acf?.masthead?.cardList.map((card) => ({
                            ...card,
                            icon: {
                                src: card.icon?.url || ''
                            }
                    ...
                    section5: {
                        cardList: data?.acf?.section5?.cardList.map((card) => ({
                            ...card,
                            icon: {
                                ...(card?.icon && formatImage(card.icon))
                            }
                        }))
                    },
                    section6: {
                        ...data?.acf?.section6,
                        cardList: data?.acf?.section6?.cardList.map((card) => ({
                            ...card,
                            image: {
                                ...(card?.image && formatImage(card.image))
                            },
                            shortDescription: stripInitialTags(card.shortDescription),
                            descriptions: convertArrayOfObjectsToStrings(card?.descriptions).map((item) =>
                                stripInitialTags(item)
                            )
                        }))
                    },
